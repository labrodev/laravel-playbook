...
The playbook ships as **13 atomic skills** under [`skills/`](skills/) — each one self-contained, covering one architecture component or aspect, installable individually or all at once via Laravel Boost:
...
| `labrodev-action` | Actions, Services, domain Rules, Orchestrators (write side) |
...
| `labrodev-workflow` | Task workflow, review checklist, tooling gates, recipes |

The original monolithic skill remains at [`skills/labrodev-playbook/`](skills/labrodev-playbook/) (docs chapters + stubs — the atomic skills embed their templates from it) and will be deprecated once the atomic set is stable.
