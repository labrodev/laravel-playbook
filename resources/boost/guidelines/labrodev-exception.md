...
- No catching-and-swallowing without a stated reason — a caught exception is re-thrown (wrapped in a named exception where the layer demands it) or logged with explicit justification; never a silent empty `catch {}`.
- **No `Illuminate\Validation\ValidationException` outside the Data validation layer** — a business-gate failure is a business exception with its own class, never `ValidationException::withMessages(...)` (→ labrodev-action). How a domain exception surfaces to the user (toast, JSON error, status code) is decided at the delivery layer via Laravel's exception handler — never by converting it to a validation error in Core.
