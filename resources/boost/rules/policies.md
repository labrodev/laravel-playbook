...
- The policy enforces the can-trio: `create()` → `{Model}Rule::canCreate(...)`, `update()` → `canUpdate($model)`, `remove()` → `canRemove($model)`; Actions never re-run these gates.
- Update/remove methods layer checks as: non-null user → ownership/scope → `{Model}Rule` gate.
- Rule gates are delegated to `{Model}Rule::...` (called statically, not via the model), never re-implemented in the policy.
- Every model has a `{Model}Policy` (mandatory per-model trio → rules/models.md).
